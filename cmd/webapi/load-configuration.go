...
type WebAPIConfiguration struct {
	...
	Web struct {
		...
		SecretKey       string        `conf:"default:the answer to life the universe and everything"`
	...
	DB    struct {
		Filename string `conf:"default:/home/charm/Programming/Project 1/WASAPhoto/db/decaf.db"` //`conf:"default:/tmp/decaf.db"`
	}
}
...
func loadConfiguration() (WebAPIConfiguration, error) {
	...
	if err := conf.Parse(os.Args[1:], "CFG", &cfg); err != nil {
		if errors.Is(err, conf.ErrHelpWanted) {
			...
			log.Println(usage) //nolint:forbidigo
